...
const determineTokenCategory = (veevaTokens) => {
  ...
  veevaTokens.forEach((veevaToken) => {
    ...
    if (isTokenContentCategory(token)) {
      ...
      setContentTypePreviewTag(veevaToken)
    }
      ...
      setEmailFragmentPreviewTag(veevaToken)
    }
      ...
      setUserInputPreviewTag(veevaToken)
    }
    // else veevaToken.category = CATEGORY_TYPES.UNKNOWN
  })
}

const getVeevaTokens = (bodyTagElement) => {

  if (!bodyTagElement) return false

  const tokenList = [];

  // Find all text nodes in bodyTagElement recursively.
  ...
  const textNodes = findTextNodes(bodyTagElement);
  textNodes.forEach(textNode => {
    ...
    REGEX_VEEVA_TOKEN.lastIndex = 0;
    while ((match = REGEX_VEEVA_TOKEN.exec(text)) !== null) {
      // Text before the token
      if (match.index > lastIndex) {
        newNodes.push(document.createTextNode(text.substring(lastIndex, match.index)));
      }

      // Create the span-wrapped token
      const span = document.createElement('span');
      span.setAttribute('data-veeva-token', '');
      span.textContent = match[0];
      newNodes.push(span);

      // Add to tokenList for downstream processing
      tokenList.push({
        token: match[0],
        parentNode: parent,
        spanElement: span,
...
const wrapVeevaTokens = (veevaToken) => {

  if (!veevaToken || !veevaToken.length) return false

  veevaToken.forEach(veevaToken => {
    veevaToken.spanElement.innerHTML = `${veevaToken.previewTag}<span data-original-token>${veevaToken.token}</span>`
  })
}